...
- `Runtime::install_deno_core_compat` で opt-in の Deno shim を入れ、`Deno.core.op*` / `ops*` に加えて `encode` / `decode` / `resources` / `close` / `tryClose` / `refOpPromise` などの utility helper と、`Deno.args` / `Deno.build` / `Deno.version` / `Deno.inspect` / `Deno.cwd` / `Deno.execPath` を最小実装で使えます
