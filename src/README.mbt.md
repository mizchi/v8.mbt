...
- `Runtime::register_async_json_op` / `take_async_json_op` / `resolve_async_json_op` / `reject_async_json_op` で `MoonBit.opAsync(...)` を host loop で処理できます
- `Runtime::register_async_bytes_op` / `take_async_bytes_op` / `resolve_async_bytes_op` / `reject_async_bytes_op` で `MoonBit.opAsyncBytes(...)` を host loop で処理できます
- `Runtime::eval_module_handle_async_string` / `eval_module_handle_async_string_with_specifier` と `ModuleEvalHandle::await_ready` で top-level await module を host loop で扱えます
...
- async host callback と richer host op surface は未実装です
