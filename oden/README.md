...
最後の 4 行は self-host smoke 用です。`oden` 自体は `mizchi/v8` の上に載る native module なので、この repo の中で `run/check/test/bundle/info/task` を試すときは明示的に `--target native` override を渡っています。将来的に別 repo へ切り出した後は、JS/WASM first な project を対象に同じ router をそのまま使う想定です。
