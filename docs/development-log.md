...
- `oden/` now exists as a sibling MoonBit module with its own `moon.mod.json`, so runtime/CLI experiments can be developed separately on top of `mizchi/v8`; its current MoonBit-first router covers `run` / `check` / `test` / `bundle` / `fmt` / `info` / `task` plus help/version/manifest
