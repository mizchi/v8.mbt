...
- `oden/` 配下には root とは別の `moon.mod.json` を持つ sibling module を切ってあり、`mizchi/v8` を local path dependency として使いながら CLI 層を別管理できる。現状の MoonBit-first router は `run` / `check` / `test` / `bundle` / `fmt` / `info` / `task` と help/version/manifest を持つ
